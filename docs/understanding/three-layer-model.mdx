...
// Human-facing app sends an intent to a node
const result = await os.use(
  new oAddress('o://company/finance/analyst'),
  {
    method: 'intent',
    params: { intent: 'Analyze Q4 2024 revenue trends' },
  }
);
...
//   trends: ['Holiday surge in December', 'B2B deals closed in November']
// }
```
...
async function analyzeBusinessHealth() {
  // Discover available nodes via the leader-hosted registry
  const matches = await node.use(
    new oAddress('o://leader/registry'),
    {
      method: 'search',
      params: { capability: 'revenue_analysis' }
    }
  );

  // Coordinate multi-tool workflow
  const revenue = await node.use(
    new oAddress('o://company/finance/analyst'),
    {
      method: 'intent',
  ...
  const churn = await node.use(
    new oAddress('o://company/finance/churn'),
    {
      method: 'intent',
...
<CardGroup cols={2}>
  ...
  <Card title="Discovery & Registry" icon="magnifying-glass" color="#0D9373">
    **Leaders** - Leader-hosted registry, capability search, and routing
  </Card>
...
```
┌─────────────────────────────────────────────────────────┐
│  LAYER 1: Agent (Human-facing app)                      │
│                                                          │
│  node.use(o://.../analyst, {                            │
│    method: 'intent',                                     │
│    params: { intent: 'Analyze Q4 2024 revenue trends' } │
...
│                                                          │
│  1. Request enters the OlaneOS runtime                  │
...
│                                                          │
│  Autonomous execution (lane loop):                      │
...
│  6. EXECUTE: Call _tool_generate_report                 │
...
// Both human-facing apps and AI agents use the same SDK call
const result = await os.use(
  new oAddress('o://company/finance/analyst'),
  {
    method: 'intent',
...
// Layer 1: Human-facing app
// → SDK: await node.use(new oAddress('o://...'), { method: 'intent', params: {...} })
