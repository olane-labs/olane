...
<Steps>
  ...
  <Step title="Add an MCP server">
    ...
    await bridge.use(new oAddress('o://mcp'), {
  ...
  <Step title="Use MCP tools">
    ...
    const result = await node.use(new oAddress('o://filesystem'), {
      method: 'read_file',
...
```typescript
// Find all MCP nodes via the leader-hosted registry
const nodes = await node.use(new oAddress('o://leader/registry'), {
  method: 'search',
  params: { type: 'TOOL' }
});
const mcpNodes = nodes.result.data.filter(n => n.address.startsWith('o://filesystem'));

// Get tools from MCP node
const tools = await node.use(new oAddress('o://filesystem'), {
  method: 'get_tools'
});

console.log(tools.result.data);
...
await agent.use(new oAddress('o://assistant'), {
  method: 'intent',
  params: {
...
Humans access the same MCP tools through the SDK. List a node's tools, then
invoke one - every interaction goes through `node.use()`:

```typescript
// List the tools a node exposes
const tools = await node.use(new oAddress('o://filesystem'), {
  method: 'get_tools'
});
console.log(tools.result.data);
...
const agent = new oLaneTool({ address: new oAddress('o://coordinator') });
await agent.use(new oAddress('o://coordinator'), {
  method: 'intent',
  params: {
...
// Discover all MCP integrations via the leader-hosted registry
const mcpTools = await node.use(new oAddress('o://leader/registry'), {
  method: 'search',
  params: { addressPrefix: 'o://integrations/mcp' }
});
```
...
const searchResult = await bridge.use(new oAddress('o://mcp'), {
  method: 'search',
  params: {
...
console.log(searchResult.result.data);
// "Found @modelcontextprotocol/server-puppeteer..."

// Validate a URL before adding
const validation = await bridge.use(new oAddress('o://mcp'), {
  method: 'validate_url',
  params: {
...
// Add if valid
if (validation.result.data.includes('valid')) {
  await bridge.use(new oAddress('o://mcp'), {
    method: 'add_remote_server',
...
const result = await analyst.use(new oAddress('o://analyst'), {
  method: 'intent',
  params: {
    intent: `
...
```typescript Usage: Human via SDK
// Humans access the same MCP tools through node.use()

// List available nodes via the leader-hosted registry
const nodes = await node.use(new oAddress('o://leader/registry'), {
  method: 'search',
  params: { type: 'TOOL' }
});
...
await node.use(new oAddress('o://filesystem'), {
  method: 'read_file',
  params: { path: './data/research/paper1.pdf' }
});

// Query database through MCP
await node.use(new oAddress('o://database'), {
  method: 'query',
  params: { sql: "SELECT * FROM findings WHERE topic='agents'" }
});

// Search for research through MCP
await node.use(new oAddress('o://research-mcp'), {
  method: 'search_papers',
  params: { query: 'agentic operating systems' }
});
```
</CodeGroup>
...
await bridge.use(new oAddress('o://company/integrations/mcp'), {
  method: 'add_local_server',
  params: {
...
const whoami = await node.use(new oAddress('o://filesystem'), {
  method: 'whoami'
});
console.log(whoami.result.data.tools);
...
# Check node status
$ olane status
# Look for unreachable nodes or connection errors
```
