...
```bash
# Inspect a running node and its tools
$ olane os status

# Address book / known nodes
$ olane os address
```

To call a tool, send an intent through the node via the SDK:

```typescript
import { oAddress } from '@olane/o-core';

// Call a tool via an intent
const result = await node.use(new oAddress('o://finance/analyst'), {
  method: 'intent',
  params: { intent: 'Calculate revenue for Q4 2024' },
});
...
<AccordionGroup>
  <Accordion title="handshake - Node information">
    ...
    const info = await node.use(new oAddress('o://company/finance/analyst'), {
      method: 'handshake',
    });
  ...
  <Accordion title="get_tools - Tool discovery">
    ...
    const tools = await node.use(new oAddress('o://company/finance/analyst'), {
      method: 'get_tools',
    });
  ...
  <Accordion title="get_params - Parameter schema">
    ...
    const schema = await node.use(new oAddress('o://company/finance/analyst'), {
      method: 'get_params',
  ...
  <Accordion title="register - Network registration">
    ...
    await node.use(new oAddress('o://leader'), {
      method: 'register',
      params: {
...
<Tabs>
  <Tab title="Type Validation">
    ...
    await node.use(new oAddress('o://my-node'), {
      method: 'calculate',
...
const tools = await node.use(new oAddress('o://finance/analyst'), {
  method: 'get_tools',
});
```
...
// Get current weather via an intent
const current = await node.use(new oAddress('o://services/weather'), {
  method: 'intent',
  params: { intent: 'Get the current weather for San Francisco' },
});
...
// Get 7-day forecast via an intent
const forecast = await node.use(new oAddress('o://services/weather'), {
  method: 'intent',
  params: { intent: 'Get a 7-day forecast for San Francisco' },
});
```

```typescript AI Agent Usage
// AI agent discovers and uses weather tools
const weatherAddress = new oAddress('o://services/weather');

// Get current weather
const current = await node.use(weatherAddress, {
  method: 'get_current',
  params: { location: 'San Francisco' }
});

// Get forecast
const forecast = await node.use(weatherAddress, {
  method: 'get_forecast',
  params: { location: 'San Francisco', days: 7 }
});

// Check for alerts
const alerts = await node.use(weatherAddress, {
  method: 'get_alerts',
  params: { location: 'San Francisco' }
});
```
