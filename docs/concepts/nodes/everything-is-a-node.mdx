...
const result = await anyNode.use(targetAddress, {...}); // Always the same
...
<AccordionGroup>
  ...
  <Accordion title="With Uniform Interface">
    ...
    await leader.use(targetAddress, {...});
    await worker.use(targetAddress, {...});
    await coordinator.use(targetAddress, {...});
    ```
  </Accordion>
</AccordionGroup>
...
interface NodeCommunication {
  use(address: oAddress, data: { method: string; params?: Record<string, any> }): Promise<oResponse>;
}
```
...
const result = await myNode.use(new oAddress('o://target'), {
...
const result = await myNode.use(target, { method: 'analyze', params: {...} });
```
...
for (const analyst of analysts) {
  await myNode.use(analyst.address, { method: 'analyze', params: {...} });
}
```
...
// Use as regular node
await leader.start();
await leader.use(new oAddress('o://leader'), { method: 'hello_world', params: {} });
...
// Call tools directly
await analyst.use(new oAddress('o://analyst'), { method: 'calculate_revenue', params: {...} });

// OR: Send intents
await analyst.use(new oAddress('o://analyst'), {
  method: 'intent',
...
// await serverNode.use(new oAddress('o://mobile/ios/' + deviceId), { 
...
const registeredNodes = await leader.use(new oAddress('o://leader'), {
  method: 'find_all',
  params: {}
});
...
await node.use(new oAddress('o://flexible-node'), { method: 'work', params: {} });
...
for (const node of nodes) {
  ...
  for (const peer of peers) {
    if (peer.address !== node.address) {
      await node.use(peer.address, { method: 'collaborate', params: {...} });
    }
  }
}
```
...
class MockNode extends oNode {
  async use(address: oAddress, data: { method: string; params?: Record<string, any> }) {
    return { 
      result: 'mocked',
      method: data.method 
    };
...
await authNode.use(new oAddress('o://auth'), { method: 'authenticate', params: { user } });
await dataNode.use(new oAddress('o://data'), { method: 'query', params: { sql } });
await queueNode.use(new oAddress('o://queue'), { method: 'enqueue', params: { job } });
...
```typescript
// Regular worker
await node.use(new oAddress('o://worker'), { method: 'work', params: {} });

// Leader (registry)
await node.use(new oAddress('o://leader'), { method: 'search', params: {} });

// Coordinator
await node.use(new oAddress('o://coordinator'), { method: 'intent', params: {} });

// External API
await node.use(new oAddress('o://api'), { method: 'fetch', params: {} });
...
// All nodes: await node.use(address, { method, params })
...
const capabilities = await myNode.use(node.address, {
  method: 'handshake',
  params: {}
});
...
await node.use(node.address, { method: 'work', params: {} });      // As worker
await node.use(node.address, { method: 'search', params: {} });    // As leader
```
...
await leader.use(leader.address, { method: 'search', params: {} });   // As leader
await leader.use(leader.address, { method: 'analyze', params: {} });  // As worker
```
...
class RegularNode extends oLaneTool {
  async _tool_coordinate_team(request: oRequest) {
    ...
    for (const member of team) {
      await this.use(member.address, { method: 'execute', params: {...} });
    }
  }
}
...
// Now it's both
await node.use(new oAddress('o://hybrid'), { method: 'search', params: {} });  // Leader
await node.use(new oAddress('o://hybrid'), { method: 'work', params: {} });    // Worker
```
