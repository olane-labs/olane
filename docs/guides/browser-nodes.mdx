...
Browsers use **WebSocket** and **WebTransport** (HTTP/3) to join the Olane network. Connections to nodes behind a NAT are established via a circuit relay (`o://relay`):

```
┌─────────────────────────────────────────────┐
│  Browser Tab (Node)                          │
│  • WebSocket for Server (browser ↔ server)  │
│  • WebTransport (HTTP/3) where available     │
...
The node automatically selects an available transport:

| Connection Type | Transport | Use Case |
|----------------|-----------|----------|
| Browser → Server | WebSocket | API calls, coordination |
| Browser → Server | WebTransport (HTTP/3) | Low-latency calls where supported |
| Browser → Browser | Circuit relay | P2P coordination via `o://relay` |

<Note>
  libp2p (the networking layer) handles transport selection automatically based on what's available. Connections are encrypted with Noise and multiplexed with Yamux.
</Note>
...
Add CSP headers to allow WebSocket and WebTransport connections:

```html
<meta http-equiv="Content-Security-Policy" 
      content="default-src 'self'; 
               connect-src 'self' wss://*.your-network.com https://*.your-network.com;
  ...
  private startCoordination(leaderTab: oNodeTool) {
    ...
    leaderTab.addTool('sync_state', async (request) => {
      ...
      for (const [id, tab] of this.tabs) {
        if (id !== this.leaderId) {
          await leaderTab.use(tab.address, {
            method: 'update_state',
...
class OptimizedBrowserNode extends oNodeTool {
  ...
  async use(address: oAddress, request: { method: string; params: any }) {
...
class LazyBrowserNode {
  ...
  async callTool(target: oAddress, method: string, params: any) {
    const node = await this.ensureStarted();
    return await node.use(target, { method, params });
  }
}

// Usage: Node only starts when first tool is called
const lazyNode = new LazyBrowserNode();
await lazyNode.callTool(new oAddress('o://data'), 'fetch_data', { id: 123 });
```
...
### Error: "Peer not reachable behind NAT"

**Cause**: A browser tab behind a NAT cannot accept inbound connections directly

**Solution**: Connections to nodes behind a NAT are established through a circuit relay (`o://relay`). Ensure the relay is reachable from the browser:
  ...
  <Accordion title="3. Monitor network quality">
    ...
    class NetworkMonitoringNode extends oNodeTool {
      ...
      async measureLatency() {
        ...
        await this.use(new oAddress('o://leader'), { method: 'ping', params: {} });
...
// await serverNode.use(new oAddress('o://dashboard/' + userId), { 
