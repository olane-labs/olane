...
<CardGroup cols={2}>
  ...
  <Card title="Long-Running Processes" icon="clock" color="#0D9373" href="/use-cases/long-running-processes">
    Long-running tasks with content-addressed, replayable execution runs
  </Card>
</CardGroup>
...
- Each run is content-addressed (CID), persisted, and deterministically replayable
...
<Tabs>
  ...
  <Tab title="Olane OS Approach">
    ...
    // Tool autonomously:
    // - Determines a sequence via the lane loop (evaluate -> execute -> re-evaluate)
    ...
    // Error? Retries and alternative paths within the lane loop.
    ```
  </Tab>
</Tabs>
...
**What it is**: Tasks that run hours or days, where each lane run is content-addressed (CID), persisted, and deterministically replayable.

**Key Benefits**:
- Each lane run is persisted as a content-addressed (CID) record
- Replay a previous run deterministically by CID
- Designed for long-running reliability
...
const research = await researchNode.use(
  new oAddress('o://research/market-analysis'),
  {
    method: 'intent',
    params: {
...
// As the lane loop runs (evaluate -> execute -> re-evaluate):
// - Each run is persisted as a content-addressed (CID) record
// - A completed run can be deterministically replayed by its CID
// - Progress streamed to the notification system
```

**Reliability Features**:

<AccordionGroup>
  <Accordion title="Content-Addressed Runs">
    Each lane run is persisted as a content-addressed (CID) record. A completed run can be deterministically replayed by its CID.
  </Accordion>
  
  <Accordion title="Replayable Execution History">
    Execution history is stored in content-addressed format. Replay, audit, or analyze any previous run by CID.
  </Accordion>
  ...
  <Accordion title="Bounded Lane Loop">
    The lane loop iterates evaluate -> execute -> re-evaluate -> stop, bounded by a maximum number of cycles to keep runs predictable.
  </Accordion>
</AccordionGroup>

**When to use**:
- ✅ Tasks take 1+ hours to complete
- ✅ Expensive operations where replayable, auditable runs matter
- ✅ Need real-time progress monitoring
- ✅ Reliability is critical (designed for long-running reliability)
...
const result = await node.use(
  ...
  {
    method: 'intent',
    params: { intent: 'Analyze customer churn risk for enterprise accounts' }
  }
);

// AI agent coordinates:
// → Discovers o://crm/customers tool (via leader registry)
// → Discovers o://analytics/churn tool (via leader registry)
// → Discovers o://finance/ltv tool (via leader registry)
...
Each lane run is persisted as a content-addressed (CID) record that can be deterministically replayed.

```typescript
// First execution runs the lane loop end to end
const first = await node.use(
  new oAddress('o://analytics/sales'),
  { method: 'intent', params: { intent: 'Analyze Q4 sales trends' } }
);
// → Runs the lane loop (evaluate -> execute -> re-evaluate)
// → Persists the run as a content-addressed (CID) record

// The same run can be deterministically replayed by its CID
// for audit, debugging, or reproducing the result
```

**Best for**: Repeated workflows, auditing, reproducible analysis
...
if (queueDepth > 100) {
  ...
  // Both processors register with the leader and become
  // discoverable via the leader-hosted registry
}
```
...
<CardGroup cols={2}>
  ...
  <Card title="Market Research Platform" icon="chart-line">
    Long-running research tasks with content-addressed, replayable runs. Analyzes competitors, trends, and market sizing.
    
    **Patterns**: Long-running processes, replayable runs
  </Card>
  ...
  <Card title="Customer Support Automation" icon="headset">
    AI-powered ticket triage and resolution with human escalation. Runs are content-addressed and replayable for audit.
    
    **Patterns**: Hybrid workflows, replayable runs, cost optimization
  </Card>
</CardGroup>
...
| **Multiple AI agents** | CrewAI ($$$) | Single LLM (can be cheaper than per-agent stacks) |
| **Long-running task** | Manual checkpointing | Content-addressed, replayable runs |
| **Serve humans + AI** | Separate APIs | Agent-agnostic design |
| **Evolving requirements** | Rewrite workflows | Intent-driven adaptation |
| **Cross-domain AI** | Fine-tune per domain ($$$) | Context injection (no per-domain training) |
