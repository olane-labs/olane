...
// Express goals in natural language via the SDK
const result = await olaneClient.use(
  new oAddress('o://company/analytics'),
  {
    method: 'intent',
    params: {
      intent: 'Analyze Q4 sales and identify top products',
    },
  }
);
...
**Web Interface**:
```typescript
// A web backend forwards the form submission through node.use()
const result = await olaneClient.use(
  new oAddress('o://company/analytics'),
  {
    method: 'intent',
...
const result = await node.use(new oAddress('o://company/crm'), {
  method: 'intent',
  params: {
...
// Human-initiated call sends the same intent
const result = await node.use(new oAddress('o://company/crm'), {
  method: 'intent',
  params: {
    intent: 'Find customers at risk of churning this month'
...
// A web backend forwards the intent through node.use()
const result = await node.use(new oAddress('o://company/crm'), {
  method: 'intent',
  params: {
    intent: 'Find customers at risk of churning this month'
...
```typescript Human Agent (SDK)
// Direct tool call from a human-initiated process
const result = await node.use(new oAddress('o://utilities/currency'), {
  method: 'convert_currency',
  params: {
    amount: 100,
...
Each Lane run is content-addressed (assigned a CID), persisted, and can be
deterministically replayed by its CID. This gives you an auditable record of
how an intent was accomplished, rather than an opaque one-off execution:

```typescript
// Run an intent — the resulting Lane execution is persisted under a CID
const result = await node.use(new oAddress('o://analytics/forecasting'), {
  method: 'intent',
  params: { intent: 'Generate sales forecast' },
});

// The run is recorded as a content-addressed sequence, for example:
// {
//   cid: 'bafy...',
...
// Because runs are content-addressed, a prior run can be replayed
// deterministically by its CID.
```
...
<Card title="Validate Inputs" icon="shield-check">
  Always define parameter schemas using oMethod definition files. Both human and AI agents benefit from clear validation.
</Card>
...
```typescript
// Support rep tooling sends an intent to find customer info
const result = await node.use(new oAddress('o://crm/customers'), {
  method: 'intent',
  params: {
...
const result = await node.use(new oAddress('o://data/pipeline/etl'), {
  method: 'intent',
  params: {
