  ...
  _params_forecast_revenue() {
    return {
      ...
      method: {
        ...
        required: false,
        default: 'linear',
        enum: ['linear', 'exponential', 'moving-average'],
        description: 'Forecasting method to use'
      }
    };
  }

  // Optional: Tool metadata for better discovery
  _meta_forecast_revenue() {
    return {
      description: 'Forecast future revenue based on historical data',
      examples: [
        {
          params: { historicalMonths: 6, method: 'exponential' },
          description: 'Forecast using 6 months of data with exponential smoothing'
        }
      ],
...
Use parameter schemas to validate inputs from all agents uniformly.

```typescript
class OrderProcessor extends oNodeTool {
  async _tool_create_order(request: oRequest) {
    // Validation happens automatically via _params_
  ...
  // Schema validates input from ANY agent
  _params_create_order() {
    return {
      customerId: {
        type: 'string',
        required: true,
...
<Steps>
  ...
  <Step title="Add parameter schemas">
    Define `_params_` methods for all tools to enable consistent validation.
  </Step>
