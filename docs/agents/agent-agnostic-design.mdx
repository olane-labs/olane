...
```typescript Human Agent (via SDK)
// Human expresses goal naturally (e.g. from a CLI wrapper or web UI)
const result = await node.use(new oAddress('o://crm'), {
  method: 'intent',
  params: {
...
```typescript Human - Complex Usage
...
```typescript Human Agent (via SDK)
// Direct tool invocation (e.g. from a CLI wrapper or web UI)
const result = await node.use(new oAddress('o://utilities/currency'), {
  method: 'convert',
  params: {
    amount: 100,
    from: 'USD',
...
```typescript Human Agent (via SDK)
// Human sends high-level intent (e.g. from a CLI wrapper or web UI)
const result = await node.use(new oAddress('o://analytics/customers'), {
  method: 'intent',
  params: {
...
// Tool node autonomously runs the lane loop:
// 1. EVALUATE: "Need customer data with purchase history"
// 2. EXECUTE: Call _tool_get_customers
// 3. EVALUATE: "Need to calculate LTV for each"
// 4. EXECUTE: Call _tool_calculate_ltv for top customers
// 5. EVALUATE: "Need churn predictions"
// 6. EXECUTE: Call _tool_predict_churn
...
for (const customer of result.atRiskCustomers) {
  await node.use(new oAddress('o://crm'), {
    method: 'create_task',
    params: {
...
<Note>
  The tool node processes intents identically regardless of source. The lane loop (EVALUATE → EXECUTE → re-EVALUATE) works the same for human and AI agents.
</Note>
...
const revenue = await node.use(new oAddress('o://finance/analyst'), {
  method: 'get_revenue',
  params: { period: 'Q4-2024' }
});
```
...
// Intent for complex coordination
const analysis = await node.use(new oAddress('o://finance/analyst'), {
  method: 'intent',
  params: {
    intent: 'Compare Q4 profitability vs Q3 and explain changes'
  }
...
**Human support rep** (via SDK from a CLI wrapper or web UI):
```typescript
// Quick customer lookup during call
await node.use(new oAddress('o://support/customer-service'), {
  method: 'get_customer',
...
class AnalystDashboard {
  async generateQuarterlyReport() {
    ...
    const result = await node.use(new oAddress('o://finance/reporting'), {
      method: 'intent',
      params: {
...
**Data engineer (manual trigger)** (via SDK from a CLI wrapper or web UI):
```typescript
// Engineer tests pipeline manually
await node.use(new oAddress('o://data/etl-pipeline'), {
  method: 'intent',
  params: {
...
class PipelineOrchestrator extends AIAgent {
  async orchestrateDaily() {
    ...
    for (const pipeline of pipelines) {
      const result = await node.use(pipeline.address, {
        method: 'intent',
        params: {
  ...
  async remediateFailed(pipeline: any, result: any) {
    ...
    await node.use(new oAddress('o://remediation'), {
      method: 'intent',
      params: {
...
class SmartOrderProcessor {
  async processOrder(order: Order) {
    // AI analyzes order
    const analysis = await node.use(new oAddress('o://orders/approval'), {
    ...
    if (analysis.needsApproval) {
      ...
      if (humanDecision.approved) {
        ...
        await node.use(new oAddress('o://orders/approval'), {
...
const result = await node.use(new oAddress('o://procurement'), {
  method: 'intent',
  params: {
...
if (humanDecision.approved) {
  await node.use(new oAddress('o://procurement'), {
...
describe('CustomerAnalytics - Agent-Agnostic', () => {
  ...
  it('should handle direct tool calls from any agent', async () => {
    const response = await toolNode.use(toolNode.address, {
    ...
    expect(response.result.data.customers).toBeDefined();
    expect(Array.isArray(response.result.data.customers)).toBe(true);
  });
  ...
  it('should handle intents from any agent', async () => {
    ...
    expect(response.result.data.atRiskCustomers).toBeDefined();
    expect(response.result.data.totalValue).toBeGreaterThan(0);
  });
  ...
  it('should return JSON-serializable results', async () => {
    ...
    const serialized = JSON.stringify(response.result.data);
    const deserialized = JSON.parse(serialized);
    
    expect(deserialized).toEqual(response.result.data);
  });

  // Test 4: Parameter validation works for all agents
  it('should validate parameters consistently', async () => {
    await expect(
      toolNode.use(toolNode.address, {
        method: 'get_customers',
