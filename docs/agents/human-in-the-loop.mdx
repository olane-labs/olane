...
const result = await node.use(
  new oNodeAddress('o://purchasing/agent'),
  {
...
if (riskScore < 0.3) {
  ...
  const review = await node.use(
    new oAddress('o://approvals/team-lead'),
    {
      method: 'intent',
      params: {
  ...
  const review = await node.use(
    new oAddress('o://underwriting/senior'),
    {
      method: 'intent',
      params: {
...
if (diagnosis.confidence > 0.9 && diagnosis.complexity === 'routine') {
  ...
  const physicianReview = await node.use(
    new oAddress('o://physician/reviewer'),
    {
      method: 'intent',
      params: {
...
<Steps>
  ...
  <Step title="Action and record-keeping">
    Decision executed and recorded for audit, with human decisions available for later analysis
  </Step>
</Steps>
...
if (moderation.violationConfidence > 0.95) {
  ...
  const review = await node.use(
    new oAddress('o://moderation/reviewer'),
    {
      method: 'intent',
      params: {
  ...
  // Persist the decision for audit and later analysis
...
<Steps>
  <Step title="Verify node is running">
    ```bash
    # Check OS and node status
    olane os status
    
    # Should report the running OS and registered nodes
    ```
  </Step>
...
<Tabs>
  ...
  <Tab title="Distribute across reviewers">
    ```typescript
    // Multiple human agents in rotation
    const humanAgents = [
      new oHumanLoginTool({ address: new oNodeAddress('o://human/alice') }),
      new oHumanLoginTool({ address: new oNodeAddress('o://human/bob') }),
      new oHumanLoginTool({ address: new oNodeAddress('o://human/charlie') })
    ];
...
- **Response time**: How quickly humans respond; set tighter targets for critical requests than for routine ones
- **Approval rate**: % of requests approved; an unusually high or low rate can indicate poor routing
- **AI-human agreement**: % where AI and human agree; a low rate is a signal to improve AI routing or analysis
- **Override rate**: % where a human overrides a high-confidence AI recommendation; a rising rate is worth investigating
