...
```bash
# Install the Olane CLI globally
pnpm add -g olane
...
The `olane` CLI focuses on workspace and OS management. Its top-level commands include `login`, `logout`, `setup`, `upgrade`, `index`, `status`, `watch`, `search`, `copass`, `ingest`, `config`, `transcript`, and `os`. Use `olane os start|stop|status|address|contacts|world|identity` to manage a local OS instance. To send natural-language intents to a tool node, use the SDK's `use()` method as shown below.

### Basic usage

#### Sending intents

Express goals in natural language. Intents are sent to a tool node using the SDK's `use()` method with the `intent` method:
...
const result = await os.use(
  ...
  {
    method: 'intent',
    params: { intent: 'Identify top-performing products' }
  }
);

// With context
const atRisk = await os.use(
  new oAddress('o://crm/customers'),
  {
    method: 'intent',
    params: {
      intent: 'Find customers at risk of churning',
...
For simple operations, call a specific tool method directly instead of routing through an intent:

```typescript
import { oAddress } from '@olane/o-core';

// Call specific tool method with parameters
...
// Response (accessed via result): { amount: 85, currency: 'EUR', rate: 0.85 }
```

#### Managing a local OS

Use the `olane os` subcommands to manage a local OS instance from the terminal:

```bash
# Start a local OS instance
olane os start

# Check OS status
olane os status

# Stop the OS instance
olane os stop
```
...
Watch long-running operations in real-time by passing a progress callback to `use()`:
...
const result = await os.use(
  ...
  {
    method: 'intent',
    params: { intent: 'Process Q4 customer data', streamTo: 'inline' }
  },
  {
    onProgress: (update) => {
...
Wrap intent calls in a small script to automate recurring work:

```typescript
// daily-report.ts - Generate a daily sales report
import { oAddress } from '@olane/o-core';
...
const result = await os.use(
  ...
  {
    method: 'intent',
    params: { intent: `Generate daily sales summary for ${today}` }
  }
);

if (result.result.success) {
  console.log('Report generated:', result.result.data);
} else {
  console.error('Report generation failed:', result.result.error);
}
```
...
<Check>
  **Use direct method calls for simple lookups**: Faster when you know the exact tool method
</Check>
...
<Check>
  **Wrap frequently used calls in scripts**: Automate repetitive tasks
</Check>
...
// Discovery endpoint (queries the leader-hosted registry)
app.get('/api/nodes', async (req, res) => {
  const nodes = await os.use(
    new oAddress('o://leader/registry'),
    { method: 'find_all', params: {} }
  );
  res.json({ nodes: nodes.result.data });
});
...
```bash
pnpm add @olane/os @olane/o-core
```
...
Find and use nodes at runtime by querying the leader-hosted registry (`o://leader/registry`). Capability and address lookup is served from the leader's in-memory registry:

```typescript
// Search the registry for matching nodes
const response = await os.use(
  new oAddress('o://leader/registry'),
  {
    method: 'search',
    params: { staticAddress: 'o://company/finance/analyst' }
  }
);

const nodes = response.result.data;
...
### Example 1: Support rep looking up a customer

A support tool quickly looks up customer information via an intent:

```typescript
import { oAddress } from '@olane/o-core';

// During a customer call, the rep's tool needs quick info
const lookup = await os.use(
  new oAddress('o://crm/customers'),
  {
    method: 'intent',
    params: { intent: 'Find customer john@example.com' }
  }
);
...
//   },
//   recentTickets: [
//     { id: 'ticket_456', subject: 'Billing question', status: 'resolved', created: '2024-09-28' }
//   ],
//   lastPurchase: '2024-09-15'
// }

// Follow up with another intent
const history = await os.use(
  new oAddress('o://crm/customers'),
  {
    method: 'intent',
    params: { intent: 'Show purchase history for cust_123' }
  }
);
```
...
**Solution**:
```bash
# Check if the local OS is running
olane os status

# Start the OS if stopped
olane os start
```

If you are connecting programmatically, verify the leader address and any
`network.connect` multiaddrs passed to `oNodeClient`.

### Error: "Node not found"

**Cause**: Tool node not registered or incorrect address

**Solution**: Query the leader-hosted registry to confirm the node is registered and check the exact address spelling:
...
**Solution**: Pass a progress callback to `use()` to observe progress, or break the work into smaller direct method calls:

```typescript
// Observe progress
await os.use(
  new oAddress('o://node'),
  { method: 'intent', params: { intent: 'Complex task', streamTo: 'inline' } },
  { onProgress: (update) => console.log(update.message) }
);

// Or break into smaller direct method calls
await os.use(new oAddress('o://node'), { method: 'tool_1', params: { /* ... */ } });
await os.use(new oAddress('o://node'), { method: 'tool_2', params: { /* ... */ } });
```
